...
import type { ChannelAccountStructure } from '~/services'

ChartJS.register(CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend)

interface Props {
  structure: ChannelAccountStructure[]
}
...
const chartData = computed(() => {
  if (!props.structure.length) return null

  return {
    labels: props.structure.map(s => s.channelName),
    datasets: [
      {
        label: '使用中',
...
<template>
  <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-semibold">渠道账户结构</h3>
